...
class VideoReconstructorHybridV2:
    ...
    def extract_audio_fingerprint(self, video_path: Path) -> Tuple[np.ndarray, float]:
        ...
        # 提取音频（使用更高的采样率以提高匹配准确性）
        audio_sample_rate = self.config.get("audio_sample_rate", 16000)  # 默认16kHz
        cmd = [
            'ffmpeg', '-y', '-hide_banner',
            '-i', str(video_path),
            '-vn',  # 无视频
            '-acodec', 'pcm_s16le',
            '-ar', str(audio_sample_rate),  # 使用配置的采样率
            '-ac', '1',  # 单声道
            '-af', 'highpass=200,lowpass=4000',  # 滤波，保留人声频率范围
            str(temp_audio)
        ]
    ...
    def find_audio_matches(self, target_fingerprints: np.ndarray,
        ...
        matches = []
        window_sizes = [5, 10, 15]  # 多尺度窗口大小
        audio_match_threshold = self.config.get("audio_match_threshold", 0.5)
        
        # 为每个目标时间点找到最佳匹配
        for target_idx in range(len(target_fingerprints)):
            best_match = None
            best_score = 0.0
            
            for window_size in window_sizes:
                if target_idx + window_size > len(target_fingerprints):
                    continue
                ...
                for source_video, source_fps in source_fingerprints_dict.items():
                    ...
                    # 滑动窗口匹配，步长为2以提高效率
                    for source_idx in range(0, len(source_fps) - window_size, 2):
                        ...
                        if similarity > best_score and similarity > audio_match_threshold:
        ...
        # 去重：合并时间相近的匹配点
        if matches:
            matches.sort(key=lambda m: m.target_time)
            deduplicated = [matches[0]]
            for match in matches[1:]:
                if match.target_time - deduplicated[-1].target_time > 1.0:  # 间隔大于1秒才保留
                    deduplicated.append(match)
                elif match.confidence > deduplicated[-1].confidence:  # 保留相似度更高的
    ...
    def verify_matches_with_video(self, audio_matches: List[AudioMatch],
        ...
        base_match_threshold = self.config.get('match_threshold', 0.25)  # 基础匹配阈值
        pre_search_frames = self.config.get('pre_search_frames', 15)  # 增加搜索范围
        search_window = self.config.get('search_window', 10)  # 搜索窗口大小
        ...
        for target_idx, target_feat in enumerate(target_features):
            ...
            for source_video, source_features in source_features_dict.items():
                ...
                for region in candidate_regions[source_video]:
                    ...
                    adaptive_window = int(search_window * (1 + confidence))  # 高置信度扩大搜索范围
                    
                    search_end_idx = min(original_idx + adaptive_window, len(source_features))
                    search_start_idx = max(0, search_start_idx - 5)  # 向前多搜索5帧
            ...
            if all_similarities:
                ...
                # 动态阈值：平均值 + 0.5倍标准差，但不低于基础阈值
                dynamic_threshold = max(base_match_threshold, mean_sim + 0.5 * std_sim)
    ...
    def segment_matches(self, matches: List[Optional[FrameMatch]]) -> List[VideoSegment]:
        ...
        min_duration = self.config.get('min_segment_duration', 0.05)
        max_gap = self.config.get('max_gap', 8.0)
        smart_fill = self.config.get('smart_fill', True)  # 是否启用智能填充
        max_none_gap = self.config.get('max_none_gap', 2)  # 允许的最大连续未匹配帧数

        print(f"\n📦 聚合片段参数: fps={fps}, min_duration={min_duration}s, max_gap={max_gap}s")
        print(f"   智能填充: {smart_fill}, 最大未匹配间隔: {max_none_gap}帧")

        last_target_time = None
        none_count = 0  # 连续未匹配帧计数

        for idx, match in enumerate(matches):
            if match is None:
                none_count += 1
                # 如果启用了智能填充且未匹配帧数不超过阈值，继续当前片段
                if smart_fill and none_count <= max_none_gap and current_segment:
                    # 尝试使用前一帧的信息填充
                    if current_segment:
                        last_match = current_segment[-1]
                        # 创建一个填充匹配，使用前一帧的源视频和时间戳
                        filled_match = FrameMatch(
                            target_frame_idx=idx,
                            source_video=last_match.source_video,
                            source_frame_idx=last_match.source_frame_idx + 1,
                            similarity=last_match.similarity * 0.9,  # 略微降低相似度
                            timestamp=last_match.timestamp + (1/fps)
                        )
                        current_segment.append(filled_match)
                        last_target_time = idx / fps
                        continue
    ...
    def check_coverage(self, segments: List[VideoSegment], target_duration: float) -> Tuple[bool, float]:
        ...
        min_coverage = self.config.get("min_coverage", 0.8)
    ...
    def generate_ffmpeg_commands(self, segments: List[VideoSegment], output_path: str,
        ...
        for idx, seg in enumerate(sorted_segments):
            ...
            duration = seg.end_time - seg.start_time
            cmd = f'ffmpeg -y -hide_banner -ss {seg.start_time:.3f} -t {duration:.3f} -i "{seg.source_video}" -an -c:v copy "{temp_file}"'
            commands.append(cmd)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='视频混剪重构工具 - 音频+视频混合对齐版 V2')
    ...
    parser.add_argument('--min-coverage', type=float, help='最小覆盖率要求')
    ...
    reconstructor = VideoReconstructorHybridV2(target_video, source_videos, config)
