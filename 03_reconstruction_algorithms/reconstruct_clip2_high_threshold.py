...
# 提高相似度阈值
SIMILARITY_THRESHOLD = 0.995  # 99.5% 阈值
...
def find_best_match_high_threshold(target_frame, source_video_path, threshold=0.995):
    """
    在源视频中查找与目标帧最匹配的帧（高阈值版本）
    只返回超过阈值的匹配
    """
...
def find_segments_with_high_threshold(clip1_path, source_videos):
    """
    分析clip1，在源视频中查找每个片段的来源（高阈值版本）
    """
    print("="*60)
    print(f"🔍 高阈值分析 (阈值: {SIMILARITY_THRESHOLD:.1%})")
    print("="*60)
    
    # 提取clip1的关键帧
    print("\n1. 提取clip1的关键帧...")
    clip1_frames = extract_keyframes(clip1_path, 12)
    ...
    for seg_idx, segment_frames in enumerate(segments):
        ...
        for source_path in source_videos:
            time, sim = find_best_match_high_threshold(
                target_frame, source_path, SIMILARITY_THRESHOLD
            )
            ...
            if time >= 0:  # 找到超过阈值的匹配
                ...
                print(f"   在 {source_path.name}: 未找到超过{SIMILARITY_THRESHOLD:.1%}的匹配 ❌")
        ...
        if best_source:
            ...
            print(f"   ❌ 未找到超过阈值的匹配，使用默认时间戳")
            # 使用默认时间戳
            default_times = [(0, 15), (30, 45), (60, 75)]
            default_source = source_videos[seg_idx % len(source_videos)]
            start, end = default_times[seg_idx]
            results.append((default_source, start, end, 0.0))
...
def create_clip2_from_segments(segments_info):
    ...
    temp_dir = OUTPUT_DIR / "temp_reconstruct_high"
...
def main():
    print("\n" + "="*60)
    print("🎬 高相似度智能视频重构工具")
    print(f"   相似度阈值: {SIMILARITY_THRESHOLD:.1%}")
    print("="*60 + "\n")
    ...
    segments_info = find_segments_with_high_threshold(CLIP1_PATH, source_videos)
