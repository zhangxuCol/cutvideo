...
class VideoReconstructorHybridV6Optimized:
    ...
    def __init__(self, target_video: str, source_videos: List[str], config: dict = None):
        ...
        # 优化参数：降低精度换取速度
        self.fps = config.get('fps', 0.5) if config else 0.5  # 降低到 0.5fps
        self.similarity_threshold = config.get('similarity_threshold', 0.85) if config else 0.85
        self.min_segment_duration = config.get('min_segment_duration', 1.0) if config else 1.0
        ...
        # 新增：快速模式参数
        self.audio_skip_threshold = 0.3  # 低于此音频分数直接跳过视频验证
        self.search_step = 5  # 音频搜索步长（秒）
    ...
    def _find_audio_match_fast(self, target_fp: np.ndarray, source_video: Path) -> List[Tuple[float, float, float]]:
        """快速音频匹配 - 返回多个候选位置"""
        ...
        for start in range(0, len(source_fp) - len(target_fp) + 1, self.search_step):
            ...
            if score > self.audio_skip_threshold:  # 只保留高分候选
                candidates.append((start, score, source_video))
    ...
    def find_best_single_source_match(self, target_duration: float):
        ...
        for source in self.source_videos:
            ...
            # 快速音频匹配，获取候选位置
            candidates = self._find_audio_match_fast(target_audio, source)
            
            for start_time, audio_score, _ in candidates:
                # 快速视频验证：只检查3个关键点
                sample_times = [0, target_duration * 0.5, min(target_duration * 0.9, target_duration - 1)]
                video_scores = []
                
                for t in sample_times:
                    if t < target_duration:
                        target_frame = self.temp_dir / f"target_{t:.0f}.jpg"
                        source_frame = self.temp_dir / f"source_{source.stem}_{start_time:.0f}_{t:.0f}.jpg"
                ...
                print(f"   {source.name} @{start_time:.0f}s: 音频{audio_score:.1%} 视频{video_score:.1%} 综合{combined_score:.1%}")
    ...
    def find_multi_source_segments_fast(self, target_duration: float) -> List[VideoSegment]:
        """快速多源片段匹配 - 降低精度"""
        print(f"\n🔍 阶段2: 多源片段拼接 (快速模式)...")
        ...
        for match in sorted(matches, key=lambda x: x['target_time']):
            if current_segment is None:
                current_segment = {
                  ...
                  abs(match['source_time'] - current_segment['end_time']) < 4):
                current_segment['end_time'] = match['source_time'] + 2
                current_segment['scores'].append(match['score'])
            else:
                if len(current_segment['scores']) >= 1:
                    segments.append(VideoSegment(
                        ...
                        target_end=current_segment['target_start'] + len(current_segment['scores']) * 2
                    ))
        ...
        if current_segment and len(current_segment['scores']) >= 1:
            segments.append(VideoSegment(
                source_video=current_segment['source'],
                start_time=current_segment['start_time'],
                end_time=current_segment['end_time'],
    ...
    def reconstruct(self, output_path: str, use_target_audio: bool = True) -> List[VideoSegment]:
        ...
        try:
            ...
            if single_match and single_match['combined_score'] > 0.85:
                ...
                segments = self.find_multi_source_segments_fast(target_duration)
